...
#divide data into two files:
fg=open('supernovagt07.dat','w')
fs=open('supernovast07.dat','w')
for datum in data:
    if (math.log10(float(datum[1]))>-0.7):
        fg.write(datum[0]+' '+datum[1]+' '+datum[2]+' '+datum[3]+'\n')
    else:
        fs.write(datum[0]+' '+datum[1]+' '+datum[2]+' '+datum[3]+'\n')
fg.close()
fs.close()
#end
...
hubble=(3*10**8)/10**(fit[0]/5.-5.0)
print hubble
t=np.arange(-2.0,0.5,0.001)
